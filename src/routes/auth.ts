...
export async function provisionVapi(restaurant: {
    ...
    email: string;
    phone?: string;
}): Promise<{ assistantId: string; phoneNumber: string; phoneId: string; bccEmail: string }> {
    ...
    log.info({ phone: phoneNumber.number }, '✅ Phone assigned');
...
// ── Verify email ──────────────────────────────────────────────────────────────
// Returns JWT so frontend can auto-login and redirect to /onboarding
...
router.post('/verify-email', async (req: Request, res: Response) => {
    try {
        ...
        if (restaurant.is_verified) {
            ...
            return res.json({ message: 'Email already verified.', token: jwt_token, status: restaurant.status });
        }
        ...
        setImmediate(async () => {
            try {
                ...
                await emailService.sendRestaurantNotification({
                    ...
                    message: `<p>Votre assistant vocal IA est configuré.</p><p><strong>Numéro IA :</strong> ${phoneNumber}</p><p><strong>Email BCC :</strong> ${bccEmail}</p>`,
                });
            } catch (vapiError: any) {
        ...
        res.json({ message: 'Email vérifié ! Bienvenue dans TableNow.', token: jwt_token, status: 'provisioning' });
    } catch (error: any) {
...
// Returns is_new_user: true when account is created for the first time
...
router.post('/google/supabase', async (req: Request, res: Response) => {
    ...
    try {
        ...
        logger.info({ status: userRes.status, email: supabaseUser?.email }, 'Supabase user fetch');
        if (!userRes.ok) return res.status(401).json({ error: 'Invalid Supabase token', detail: supabaseUser });
        ...
        if (!restaurant) {
            ...
            let slug = name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '') || `resto-${Date.now().toString(36)}`;
            const { data: existing } = await supabase.from('restaurants').select('id').eq('slug', slug).single();
            if (existing) slug = `${slug}-${Date.now().toString(36).slice(-4)}`;

            logger.info({ name, slug }, 'Creating restaurant via Google OAuth');
            ...
            if (insertErr || !newRest) {
                logger.error({ insertErr }, 'Insert failed');
        ...
        logger.info({ id: restaurant.id, slug: restaurant.slug, is_new_user }, 'Signing token');
        ...
        res.json({ token, restaurant: safeRest, is_new_user });
    } catch (err: any) {
        logger.error({ err: err?.message }, 'Supabase token exchange error');
